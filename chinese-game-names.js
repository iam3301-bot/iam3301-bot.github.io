/**
 * GameBox 中文游戏名称映射数据库
 * 包含热门游戏的中英文对照，支持搜索优化
 * 更新时间: 2025-12-03
 */

window.chineseGameNames = {
  // === 3A大作 ===
  "Cyberpunk 2077": "赛博朋克2077",
  "The Witcher 3: Wild Hunt": "巫师3：狂猎",
  "Red Dead Redemption 2": "荒野大镖客2",
  "Grand Theft Auto V": "侠盗猎车手5",
  "Elden Ring": "艾尔登法环",
  "Sekiro: Shadows Die Twice": "只狼：影逝二度",
  "Dark Souls III": "黑暗之魂3",
  "Bloodborne": "血源诅咒",
  "God of War": "战神",
  "The Last of Us Part II": "最后生还者2",
  "Horizon Zero Dawn": "地平线：零之曙光",
  "Death Stranding": "死亡搁浅",
  "Control": "控制",
  "Resident Evil Village": "生化危机8：村庄",
  "Resident Evil 4": "生化危机4",
  "Monster Hunter: World": "怪物猎人：世界",
  "Monster Hunter Rise": "怪物猎人：崛起",
  "Final Fantasy VII Remake": "最终幻想7：重制版",
  "Final Fantasy XV": "最终幻想15",
  "Persona 5 Royal": "女神异闻录5：皇家版",
  "NieR: Automata": "尼尔：机械纪元",
  "Metal Gear Solid V": "合金装备5",
  "Assassin's Creed Valhalla": "刺客信条：英灵殿",
  "Assassin's Creed Odyssey": "刺客信条：奥德赛",
  "Watch Dogs: Legion": "看门狗：军团",
  "Far Cry 6": "孤岛惊魂6",
  "Dying Light 2": "消逝的光芒2",
  "Metro Exodus": "地铁：离去",
  "Doom Eternal": "毁灭战士：永恒",
  "Wolfenstein II": "德军总部2",
  
  // === 动作冒险 ===
  "Hollow Knight": "空洞骑士",
  "Celeste": "蔚蓝",
  "Ori and the Will of the Wisps": "奥日与鬼火意志",
  "Hades": "哈迪斯",
  "Dead Cells": "死亡细胞",
  "Cuphead": "茶杯头",
  "Undertale": "传说之下",
  "Deltarune": "三角符文",
  "Stardew Valley": "星露谷物语",
  "Terraria": "泰拉瑞亚",
  "Don't Starve Together": "饥荒联机版",
  "Subnautica": "深海迷航",
  "The Forest": "森林",
  "Valheim": "英灵神殿",
  "Green Hell": "绿色地狱",
  
  // === 角色扮演 ===
  "Baldur's Gate 3": "博德之门3",
  "Divinity: Original Sin 2": "神界：原罪2",
  "Disco Elysium": "极乐迪斯科",
  "Wasteland 3": "废土3",
  "Pathfinder: Wrath of the Righteous": "开拓者：正义之怒",
  "Dragon Age: Inquisition": "龙腾世纪：审判",
  "Mass Effect Legendary Edition": "质量效应：传奇版",
  "Skyrim": "上古卷轴5：天际",
  "Fallout 4": "辐射4",
  "Fallout: New Vegas": "辐射：新维加斯",
  
  // === 策略游戏 ===
  "Civilization VI": "文明6",
  "Total War: WARHAMMER III": "全面战争：战锤3",
  "XCOM 2": "幽浮2",
  "Stellaris": "群星",
  "Crusader Kings III": "十字军之王3",
  "Europa Universalis IV": "欧陆风云4",
  "Hearts of Iron IV": "钢铁雄心4",
  "Age of Empires IV": "帝国时代4",
  "Starcraft II": "星际争霸2",
  "Command & Conquer": "命令与征服",
  "Into the Breach": "陷阵之志",
  "Slay the Spire": "杀戮尖塔",
  
  // === 射击游戏 ===
  "Counter-Strike 2": "反恐精英2",
  "Counter-Strike: Global Offensive": "反恐精英：全球攻势",
  "Valorant": "无畏契约",
  "Apex Legends": "apex英雄",
  "Call of Duty": "使命召唤",
  "Battlefield 2042": "战地2042",
  "Rainbow Six Siege": "彩虹六号：围攻",
  "Overwatch 2": "守望先锋2",
  "Team Fortress 2": "军团要塞2",
  "Left 4 Dead 2": "求生之路2",
  "Payday 2": "收获日2",
  "Borderlands 3": "无主之地3",
  "Destiny 2": "命运2",
  "Warframe": "星际战甲",
  
  // === 模拟经营 ===
  "The Sims 4": "模拟人生4",
  "Cities: Skylines": "城市：天际线",
  "Planet Zoo": "动物园之星",
  "Two Point Hospital": "双点医院",
  "Farming Simulator 22": "模拟农场22",
  "Euro Truck Simulator 2": "欧洲卡车模拟2",
  "American Truck Simulator": "美国卡车模拟",
  "House Flipper": "房产达人",
  "PowerWash Simulator": "洗车模拟器",
  "Unpacking": "开箱",
  
  // === 多人在线 ===
  "Dota 2": "刀塔2",
  "League of Legends": "英雄联盟",
  "Genshin Impact": "原神",
  "Lost Ark": "失落的方舟",
  "Final Fantasy XIV": "最终幻想14",
  "World of Warcraft": "魔兽世界",
  "Guild Wars 2": "激战2",
  "Black Desert Online": "黑色沙漠",
  "Path of Exile": "流放之路",
  "Warframe": "星际战甲",
  "Minecraft": "我的世界",
  "Roblox": "罗布乐思",
  "Fortnite": "堡垒之夜",
  "PUBG": "绝地求生",
  "Rust": "腐蚀",
  "ARK: Survival Evolved": "方舟：生存进化",
  "DayZ": "僵尸末日",
  
  // === 恐怖生存 ===
  "Resident Evil 2": "生化危机2：重制版",
  "Resident Evil 3": "生化危机3：重制版",
  "Silent Hill": "寂静岭",
  "Outlast": "逃生",
  "Amnesia": "失忆症",
  "Alien: Isolation": "异形：隔离",
  "Dead Space": "死亡空间",
  "The Evil Within": "恶灵附身",
  "Soma": "索玛",
  "Layers of Fear": "层层恐惧",
  
  // === 赛车竞速 ===
  "Forza Horizon 5": "极限竞速：地平线5",
  "F1 2023": "F1 2023",
  "Gran Turismo 7": "GT赛车7",
  "Need for Speed": "极品飞车",
  "Dirt Rally 2.0": "尘埃拉力赛2.0",
  "Assetto Corsa": "神力科莎",
  "iRacing": "iRacing",
  
  // === 体育运动 ===
  "FIFA 23": "FIFA 23",
  "NBA 2K24": "NBA 2K24",
  "Madden NFL 24": "麦登橄榄球24",
  "WWE 2K24": "WWE 2K24",
  
  // === 格斗游戏 ===
  "Street Fighter 6": "街头霸王6",
  "Tekken 8": "铁拳8",
  "Mortal Kombat 11": "真人快打11",
  "Guilty Gear Strive": "罪恶装备：Strive",
  "Dragon Ball FighterZ": "龙珠斗士Z",
  
  // === 独立佳作 ===
  "Among Us": "我们之中",
  "Fall Guys": "糖豆人",
  "Phasmophobia": "恐鬼症",
  "Lethal Company": "致命公司",
  "Palworld": "幻兽帕鲁",
  "V Rising": "V Rising",
  "Core Keeper": "核心守护者",
  "Dave the Diver": "深海潜水员戴夫",
  "Pizza Tower": "披萨塔",
  "Vampire Survivors": "吸血鬼幸存者",
  "Brotato": "土豆兄弟",
  "20 Minutes Till Dawn": "黎明前20分钟",
  
  // === 2024-2025新游戏 ===
  "Starfield": "星空",
  "Spider-Man 2": "蜘蛛侠2",
  "Alan Wake 2": "心灵杀手2",
  "Baldur's Gate 3": "博德之门3",
  "Hogwarts Legacy": "霍格沃茨之遗",
  "Diablo IV": "暗黑破坏神4",
  "Street Fighter 6": "街头霸王6",
  "Final Fantasy XVI": "最终幻想16",
  "Resident Evil 4 Remake": "生化危机4：重制版",
  "Dead Island 2": "死亡岛2",
  "Star Wars Jedi: Survivor": "星球大战绝地：幸存者",
  "The Legend of Zelda: Tears of the Kingdom": "塞尔达传说：王国之泪",
  "Armored Core VI": "装甲核心6",
  "Lies of P": "P的谎言",
  "Lords of the Fallen": "堕落之王",
  "Payday 3": "收获日3",
  "Avatar: Frontiers of Pandora": "阿凡达：潘多拉边境",
  "Like a Dragon: Infinite Wealth": "人中之龙8",
  "Tekken 8": "铁拳8",
  "Persona 3 Reload": "女神异闻录3：重制版",
  "Final Fantasy VII Rebirth": "最终幻想7：重生",
  "Dragon's Dogma 2": "龙之信条2",
  "Rise of the Ronin": "浪人崛起",
  "Helldivers 2": "绝地潜兵2",
  "Senua's Saga: Hellblade II": "塞娜的传说：地狱之刃2",
  "S.T.A.L.K.E.R. 2": "潜行者2",
  "Black Myth: Wukong": "黑神话：悟空",
  "Assassin's Creed Shadows": "刺客信条：影",
  "Grand Theft Auto VI": "侠盗猎车手6",
  "Frostpunk 2": "冰封朋克2",
  "Manor Lords": "庄园领主",
  "Hades II": "哈迪斯2",
  "Hollow Knight: Silksong": "空洞骑士：丝之歌",
  
  // === 中国游戏 ===
  "Gujian 3": "古剑奇谭3",
  "Sword and Fairy 7": "仙剑奇侠传7",
  "Tale of Immortal": "鬼谷八荒",
  "Dyson Sphere Program": "戴森球计划",
  "ANNO: Mutationem": "ANNO：变种",
  "Bright Memory": "光明记忆",
  "Faith of Danschant": "信仰：丹尚的女儿",
  "Immortals Fenyx Rising": "渡神纪：芬尼斯崛起",
  
  // 额外映射
  "Portal 2": "传送门2",
  "Portal": "传送门",
  "Half-Life 2": "半条命2",
  "Bioshock": "生化奇兵",
  "Dishonored": "羞辱",
  "Prey": "掠食",
  "Deathloop": "死亡循环"
};

// 创建反向映射（中文->英文）
window.chineseToEnglish = {};
Object.entries(window.chineseGameNames).forEach(([en, cn]) => {
  window.chineseToEnglish[cn] = en;
});

// 获取游戏的中文名称
window.getChineseName = function(englishName) {
  return window.chineseGameNames[englishName] || englishName;
};

// 获取游戏的英文名称
window.getEnglishName = function(chineseName) {
  return window.chineseToEnglish[chineseName] || chineseName;
};

// 智能搜索：同时支持中英文
window.searchGameName = function(query, gameName) {
  if (!query || !gameName) return false;
  
  const q = query.toLowerCase().trim();
  const name = gameName.toLowerCase();
  const chineseName = (window.getChineseName(gameName) || '').toLowerCase();
  
  // 检查英文名
  if (name.includes(q)) return true;
  
  // 检查中文名
  if (chineseName && chineseName.includes(q)) return true;
  
  // 检查拼音首字母（简单实现）
  const pinyin = getPinyin(chineseName);
  if (pinyin && pinyin.includes(q)) return true;
  
  return false;
};

// 简单的拼音首字母提取（仅用于常见游戏）
function getPinyin(chinese) {
  const pinyinMap = {
    '赛博朋克': 'sbpk',
    '巫师': 'ws',
    '荒野大镖客': 'hydbk',
    '侠盗猎车手': 'xdlcs',
    '艾尔登法环': 'aedfh',
    '只狼': 'zl',
    '黑暗之魂': 'hazh',
    '血源': 'xy',
    '战神': 'zs',
    '地平线': 'dpx',
    '死亡搁浅': 'swgq',
    '生化危机': 'shwj',
    '怪物猎人': 'gwlr',
    '最终幻想': 'zzhx',
    '女神异闻录': 'nsywl',
    '尼尔': 'ne',
    '刺客信条': 'ckxt',
    '空洞骑士': 'kdqs',
    '星露谷物语': 'xlgwy',
    '泰拉瑞亚': 'tlry',
    '博德之门': 'bdzm',
    '文明': 'wm',
    '反恐精英': 'fkjy',
    '原神': 'ys',
    '英雄联盟': 'yxlm',
    '我的世界': 'wdsj',
    '黑神话': 'hsh',
    '悟空': 'wk'
  };
  
  for (const [cn, py] of Object.entries(pinyinMap)) {
    if (chinese.includes(cn)) {
      return py;
    }
  }
  return '';
}

console.log(`📚 中文游戏名称数据库加载完成: ${Object.keys(window.chineseGameNames).length} 个映射`);
